	...
	Room.prototype = {
		...
		create : function() {
			...
			var pos = 0, max = 0, prev = 0,
				wallsCount = support.transforms3d ? 15 : 1;
		...
		getLayoutSettings : function() {
			...
			var perwall = Math.floor( this.totalItems / 15 ),
				lastwall = perwall + this.totalItems % 15;
			...
			if( dir === 'next' ) {
				...
				if( support.transforms3d ) {
					if( this.currentItem < wallItemsCount - 1 ) {
						...
						// update current wall
						this.currentWall < 14 ? ++this.currentWall : this.currentWall = 0;
						// update wall
						wall = this.walls[ this.currentWall ];
						changeWall = true;
						// reset currentItem
				...
				if( support.transforms3d ) {
					if( this.currentItem > 0 ) {
						...
						this.lastWall = this.currentWall;
						// update current wall
						this.currentWall > 0 ? --this.currentWall : this.currentWall = 14;			
						// update wall
						wall = this.walls[ this.currentWall ];
						changeWall = true;
						// reset currentItem
